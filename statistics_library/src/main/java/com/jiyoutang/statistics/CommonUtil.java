...
public class CommonUtil {
    ...
    public static void onResume(Activity activity){
        String activityName = CommonUtil.getActivityName(activity);
        String saveName = CommonUtil.getPageName(activity);
        if(TextUtils.isEmpty(saveName)) {
            //应用程序application重启了
            UmsAgent.onEvent(activity, UmsConstants.newActivityStartUniqueCode, true);
            return;
        }
        if(!TextUtils.isEmpty(activityName) && activityName.equals(saveName)){
            if(CommonUtil.isNewSession(activity)){
                UmsAgent.onEvent(activity, UmsConstants.newActivityStartUniqueCode, true);
            }
        }
    }
    ...
    public static void onPause(Activity activity){
        try {
            CommonUtil.generateSession(activity);
            CommonUtil.savePageName(activity, CommonUtil.getActivityName(activity));
    ...
    public static String generateSession(Context context) throws ParseException {
        String sessionId = CommonUtil.md5Appkey(DeviceInfo.getDeviceTime());
        SharedPrefUtil sp = new SharedPrefUtil(context);
    ...
    public static String getPageName(Context context) {
        SharedPrefUtil sp = new SharedPrefUtil(context);
        return sp.getValue("CurrentPage","");
    }
